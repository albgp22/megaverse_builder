...
#[cfg(test)]
mod tests {
    use crate::megaverse::astral::objects::{AstralObject, Direction, Color};
    ...
    fn test_cometh_parsing(){
        ...
        let expected = AstralObject::Cometh { row:0, column: 1, direction: Direction::Left };
        assert_eq!(parsed,expected);
    }
    #[test]
    fn test_soloon_parsing(){
        let parsed = AstralObject::build_from_string(0, 1, "PURPLE_SOLOON".to_string()).unwrap();
        let expected = AstralObject::Soloon { row: 0, column: 1, color: Color::Purple };
        assert_eq!(parsed,expected);
    }
}